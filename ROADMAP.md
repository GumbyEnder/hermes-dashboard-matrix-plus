...
## 2026-04-12 dashboard redesign merge
- Merged redesign IA into live shell (Projects + Reports split).
- Added backend-backed project thread binding endpoint: POST /api/project/thread.
- Rewired project/report cards to backend feeds and preserved 5s polling cadence.
- Unified theme handling around redesign IDs with legacy alias compatibility.

## 2026-04-12 Live redesign merge update (2026-04-12T19:51:21Z)
- Merged redesign shell structure directly into live `/` path (`static/index.html`) with Projects-first nav ordering and sidebar rail/section split.
- Kept backend-backed Projects/Reports flow via existing ops routes; preserved project-thread bind (`/api/project/thread`) and 5s refresh cadence.
- Unified theme handling with canonical redesign themes + legacy alias normalization (`hermes-theme` persistence unchanged).
