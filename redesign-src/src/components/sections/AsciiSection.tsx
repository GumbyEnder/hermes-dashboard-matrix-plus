...
const PRESET_ARTS = [
  { id: "hermes", name: "Hermes Logo", art: HERMES_ASCII },
  { id: "agent", name: "Agent", art: `
  ┌─────────┐
  │  ◉   ◉  │
  │    ▽    │
  │  ╰───╯  │
  ...
  return (
    <LeftPanel title="ASCII Art" onRefresh={() => {}}>
      <div className="px-3 py-2 space-y-1">
        {PRESET_ARTS.map((p) => (
          <div key={p.id} className="flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-row-hover cursor-pointer transition-colors">
            <Terminal size={12} className="text-hermes-accent shrink-0" />
            <span className="text-xs">{p.name}</span>
          </div>
        ))}
        <div className="border-t border-hermes-border my-2" />
        <div className="text-[10px] text-hermes-muted px-2">Click to preview • Edit in main panel</div>
      </div>
    </LeftPanel>
  );
}
...
export function AsciiMain() {
  const [art, setArt] = useState(HERMES_ASCII);
