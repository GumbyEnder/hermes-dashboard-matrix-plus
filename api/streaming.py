...
def _run_agent_streaming(session_id, msg_text, model, workspace, stream_id, attachments=None):
    ...
    try:
        ...
        try:
            ...
            try:
                from api.dashboard_phase2 import record_dashboard_event
                for tc in tool_calls:
                    ...
                    if name == 'delegate_task':
                        ...
                        spawn_label = ''
                        if isinstance(args, dict):
                            spawn_label = str(args.get('goal') or args.get('prompt') or args.get('name') or '').strip()
                        record_dashboard_event(
                            'agent_spawned',
                            session_id=s.session_id,
                            tool_name=name,
                            child_label=spawn_label or tc.get('snippet') or 'delegated agent',
                            tool_call_id=tc.get('tid'),
                            assistant_msg_idx=tc.get('assistant_msg_idx'),
                        )
            except Exception:
