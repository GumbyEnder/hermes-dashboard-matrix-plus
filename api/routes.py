...
from api.dashboard_phase2 import (
    load_recent_obsidian_touched,
    load_briefs,
    build_dialog_graph,
...
def _resolve_dashboard_index_html() -> Path:
    candidates = [
        Path.home() / 'hermes-dashboard-redesign_src2-dist' / 'index.html',
        Path('/tmp/src2_build_workspace/dist/index.html'),
        _INDEX_HTML_PATH,
...
def handle_get(handler, parsed) -> bool:
    ...
    if parsed.path in ('/', '/index.html'):
        return t(handler, _resolve_dashboard_index_html().read_text(encoding='utf-8'),
    ...
    if parsed.path == '/favicon.ico':
        return _serve_design_asset(handler, 'favicon.ico')

    if parsed.path in ('/robots.txt', '/placeholder.svg'):
        return _serve_design_asset(handler, parsed.path.lstrip('/'))
    ...
    if parsed.path == '/api/text-file':
        qs = parse_qs(parsed.query)
        path_str = qs.get('path', [''])[0]
        if not path_str:
            return bad(handler, 'path is required')
        try:
            target = _resolve_editable_text_file(path_str)
            return j(handler, {'path': str(target), 'content': target.read_text(encoding='utf-8', errors='replace')})
    ...
    if parsed.path == '/api/skills/catalog':
        from tools.skills_tool import skills_list as _skills_list
        raw = _skills_list()
        data = json.loads(raw) if isinstance(raw, str) else raw
        skills = data.get('skills', [])
        skills = sorted(skills, key=lambda s: ((s or {}).get('category') or '', (s or {}).get('name') or ''))
        return j(handler, {'skills': skills, 'total': len(skills), 'mode': 'catalog'})

    if parsed.path == '/api/skills/requests':
        return j(handler, {'requests': _load_skill_requests_store().get('requests', [])})
    ...
    if parsed.path == '/api/profile/content':
        qs = parse_qs(parsed.query)
        name = qs.get('name', [''])[0].strip()
        if not name:
            return bad(handler, 'name is required')
        from api.profiles import list_profiles_api, _resolve_base_hermes_home
        profile = next((p for p in list_profiles_api() if p.get('name') == name), None)
        if not profile:
            return bad(handler, 'Profile not found', 404)
        base_home = _resolve_base_hermes_home()
        if name == 'default':
            profile_path = base_home.resolve()
        else:
            profile_path = (base_home / 'profiles' / name).resolve()
    ...
    if parsed.path == '/api/ops/tokens':
        total_in = total_out = total_cost = 0
        sessions = [s for s in all_sessions() if isinstance(s, dict)]
        for s in sessions:
            total_in += int(s.get('input_tokens', 0) or 0)
            total_out += int(s.get('output_tokens', 0) or 0)
            total_cost += float(s.get('estimated_cost', 0.0) or 0.0)
        return j(handler, {
    ...
    if parsed.path == '/api/ops/agents':
        try:
            cmd = "ps -eo pid,ppid,pcpu,pmem,etimes,command --sort=-pcpu | grep -i hermes | grep -v grep"
            ...
            for line in out.split('\n'):
                ...
                if len(parts) < 6:
                    continue
                pid, ppid, pcpu, pmem, etimes, command = parts
                agents.append({
                    'pid': int(pid),
                    'ppid': int(ppid),
            ...
            return j(handler, {'agents': []})

    if parsed.path == '/api/ops/projects':
        proj_dir = Path('/mnt/nas/agents/Projects/')
        nas_ok = os.path.ismount('/mnt/nas')
        projects = []
        persisted = [p for p in (load_projects() or []) if isinstance(p, dict)]
        by_name = {str((p.get('name') or '')).lower(): p for p in persisted if p.get('name')}
        by_path = {str((p.get('path') or '')).lower(): p for p in persisted if p.get('path')}
        sess = [s for s in all_sessions() if isinstance(s, dict)]
        latest_by_project = {}
        for s in sess:
            ...
            if not prev or (s.get('updated_at', 0) or 0) > (prev.get('updated_at', 0) or 0):
                latest_by_project[pid] = s
        briefs = (build_ops_briefs_payload() or {}).get('briefs', [])
        if nas_ok and proj_dir.exists():
            for p in sorted(proj_dir.iterdir(), key=lambda x: x.stat().st_mtime, reverse=True):
                if p.is_dir() and not p.name.startswith('.'):
                    stat = p.stat()
                    linked = by_path.get(str(p).lower()) or by_name.get(p.name.lower()) or {}
                    pid = linked.get('project_id') or uuid.uuid5(uuid.NAMESPACE_URL, str(p.resolve())).hex[:12]
                    thread = latest_by_project.get(pid)
                    status = linked.get('status') or ('active' if thread else 'idle')
                    url = linked.get('url') or ''
                    access = linked.get('access_instructions') or f'Open workspace: {p}'
                    brief_preview = linked.get('brief') or ''
                    if not brief_preview:
                        m = next((b for b in briefs if p.name.lower() in str((b.get('title') or '')).lower()), None)
                        if m:
                            brief_preview = str(m.get('next') or m.get('what') or '')
                    projects.append({
                        'project_id': pid,
                        'name': linked.get('name') or p.name,
                        'path': str(p),
                        'mtime': stat.st_mtime,
                        ...
                        'thread_session_id': thread.get('session_id') if thread else None,
                    })
        return j(handler, {'projects': projects[:30], 'nas_available': nas_ok})

    if parsed.path == '/api/ops/obsidian':
        touched = load_recent_obsidian_touched(limit=50, days=30)
        return j(handler, {'recent_touched': touched, 'total_files': len(touched)})

    if parsed.path == '/api/ops/briefs':
        return j(handler, build_ops_briefs_payload())

    if parsed.path == '/api/ops/ledger':
        return j(handler, build_ops_ledger_payload())

    if parsed.path == '/api/ops/dialogs':
        return j(handler, build_dialog_graph())

    if parsed.path == '/api/ops/costs':
        return j(handler, build_cost_breakdown())

    # Notes
    if parsed.path == '/api/notes':
        notes_path = Path.home() / '.hermes' / 'notes.md'
        if notes_path.exists():
            lines = notes_path.read_text(encoding='utf-8').strip().split('\\n')
            return j(handler, {'notes': lines[-50:]})  # last 50 lines
        return j(handler, {'notes': []})
...
def handle_post(handler, parsed) -> bool:
    ...
    if parsed.path == '/api/notes/save':
        ...
        return j(handler, {'ok': True, 'path': str(notes_path)})

    if parsed.path == '/api/text-file/save':
    ...
    # ── Project CRUD (POST) ──
    if parsed.path == '/api/project/thread':
        project_id = (body.get('project_id') or '').strip() if isinstance(body.get('project_id'), str) else body.get('project_id')
        project_name = str(body.get('project_name') or '').strip()
        project_path = str(body.get('project_path') or body.get('workspace') or '').strip()
        projects = [p for p in (load_projects() or []) if isinstance(p, dict)]
        proj = None
        if project_id:
            proj = next((p for p in projects if p.get('project_id') == project_id), None)
        if not proj and project_name:
            proj = next((p for p in projects if str(p.get('name') or '').lower() == project_name.lower()), None)
        if not proj and project_path:
            proj = next((p for p in projects if str(p.get('path') or '') == project_path), None)
        if not proj:
            if not project_name:
                project_name = Path(project_path).name if project_path else ''
            if not project_name:
                return bad(handler, 'project_name or project_id required')
            proj = {
                'project_id': uuid.uuid4().hex[:12],
                'name': project_name,
                'color': body.get('color') or '#4f8cff',
                'path': project_path,
            }
            projects.append(proj)
            save_projects(projects)

        bound = [s for s in all_sessions() if isinstance(s, dict) and s.get('project_id') == proj.get('project_id')]
        bound.sort(key=lambda x: x.get('updated_at', 0) or 0, reverse=True)
        created = False
        if bound:
            try:
                s = get_session(bound[0]['session_id'])
            except KeyError:
                s = new_session(workspace=proj.get('path') or project_path or None, model=body.get('model'))
                s.project_id = proj.get('project_id')
                s.save()
                created = True
        else:
            s = new_session(workspace=proj.get('path') or project_path or None, model=body.get('model'))
            s.project_id = proj.get('project_id')
            brief = str(body.get('brief') or proj.get('brief') or '').strip()
            if brief:
                s.messages.append({'role': 'assistant', 'content': f"Project brief loaded for {proj.get('name')}:\n{brief}"})
            s.save()
            created = True

        status = proj.get('status') or ('active' if not created else 'new')
        url = proj.get('url') or ''
        access = proj.get('access_instructions') or (f"Open workspace: {proj.get('path')}" if proj.get('path') else 'Use file panel to access project workspace.')
        payload = s.compact() | {'messages': s.messages}
        return j(handler, {'project': proj, 'session': payload, 'created': created, 'status': status, 'url': url, 'access_instructions': access})
    ...
    if parsed.path == '/api/projects/update':
        ...
        if not proj: return bad(handler, 'Project not found', 404)
        if 'name' in body:
            proj['name'] = str(body['name']).strip()[:128] or proj.get('name', '')
        if 'status' in body:
            proj['status'] = str(body['status']).strip()
        if 'url' in body:
            proj['url'] = str(body['url']).strip()
        if 'access_instructions' in body:
            proj['access_instructions'] = str(body['access_instructions']).strip()
        if 'brief' in body:
            proj['brief'] = str(body['brief']).strip()
            proj['brief_preview'] = str(body['brief']).strip()
        if 'color' in body:
            color = body['color']
            if color and not _re.match(r'^#[0-9a-fA-F]{3,8}$', color):
                return bad(handler, 'Invalid color format')
...
def _serve_design_asset(handler, rel_path: str):
    """Serve src2 build assets first, then fall back to the legacy static tree."""
    roots = [
        (Path.home() / 'hermes-dashboard-redesign_src2-dist').resolve(),
        Path('/tmp/src2_build_workspace/dist').resolve(),
        (Path(__file__).parent.parent / 'redesign_src2' / 'dist').resolve(),
        (Path(__file__).parent.parent / 'media_assets').resolve(),
        (Path(__file__).parent.parent / 'static').resolve(),
    ]
    rel = rel_path.lstrip('/')
    aliases = [rel]
    if rel.startswith('assets/media/'):
        aliases.append(rel[len('assets/media/'):])
    elif rel.startswith('assets/'):
        aliases.append(rel[len('assets/'):])
    for root in roots:
        for rel_candidate in aliases:
            asset = (root / rel_candidate).resolve()
            try:
...
def _resolve_editable_text_file(path_str: str) -> Path:
    allowed_suffixes = {'.md', '.markdown', '.txt', '.yaml', '.yml', '.env'}
    target = Path(path_str).expanduser().resolve()
    if not target.exists() or not target.is_file():
        raise FileNotFoundError('File not found')
    if target.suffix.lower() not in allowed_suffixes and target.name != '.env':
        raise ValueError('Unsupported file type')
    allowed_roots = [
        Path.home().resolve(),
        Path('/mnt/nas').resolve(),
