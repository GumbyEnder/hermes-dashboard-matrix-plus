...
## 2026-04-12 projects-first shell integration
- Ops panel split into Projects (ops) and Reports (usage/cost/live agents).
- Workspace right rail now renders compact system-resource mini block from /api/ops/resources.
- Project click flow calls POST /api/project/thread to resolve existing bound session or create one and preload brief context.
- Theme system canonical set: matrix,bwg,chroma,white,amber,nord,forge,terminal; legacy IDs map through normalization.

## 2026-04-12 Frontend shell architecture refresh (2026-04-12T19:51:21Z)
- Live shell now uses a rail + section-stack left pane (`.sidebar-shell`) mapped to existing panel IDs (`panelChat`, `panelOps`, `panelReports`, etc.).
- Workspace System Resources remain compact and rendered in right panel (`#workspaceResourceMini`) from `/api/ops/resources`.
- Theme application paths now normalize aliases to canonical IDs (`matrix|bwg|chroma|white|amber|nord|forge|terminal`) while preserving localStorage key `hermes-theme`.
